...
if ($t == "login") {
    ...
    if (!empty($_POST['login_id']) && !empty($_POST['login_password'])) {
		...
		//Update Adrian Garcia, para soportar las password que migramos de Vituin

		//

		$login_password_md5_old = md5($login_password.'29bca3bf8c1e04a16b3f699f5781c023'); //for
		...
		$query_one = "SELECT id FROM " . DB_ACCOUNTS . " WHERE $db_query_part AND (password='$login_password_md5' OR password='$login_password_md5_old') AND type='user' AND active=1";
        ...
        if (($sql_numrows_one = mysqli_num_rows($sql_query_one)) == 1) {
            ...
            $query_two = "SELECT username,email_verified,password FROM " . DB_ACCOUNTS . " WHERE id=" . $sql_fetch_one['id'] . " AND (password='$login_password_md5' OR password='$login_password_md5_old')AND type='user' AND active=1";
            ...
            if (($sql_numrows_two = mysqli_num_rows($sql_query_two)) == 1) {
                ...
                if ($continue == true) {
                    ...
                    $_SESSION['user_pass'] = $sql_fetch_two['password'];
...
$data['stories'] = print_r($stories,TRUE);
