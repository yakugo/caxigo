...
{
    exit("SocialKit requires atleast PHP version 5.4.0 to work. Your PHP version is: " . PHP_VERSION . ". PHP 5.4.0 was released on March <strong>2012</strong>. It's about time you make the upgrade. Please upgrade your PHP version to 5.4+ or contact your hosting provider and ask them to upgrade it to 5.4+.");
}

if (! function_exists('mysqli_connect'))
{
    exit("SocialKit requires MySQLi Extension to work. MySQLi Extension is either not installed or not loaded in your server! Please install MySQLi extension in your server or contact your hosting provider and ask them to install MySQLi extension.");
}

if (! extension_loaded('gd') && ! extension_loaded('gd2'))
{
    exit("SocialKit requires GD Library to process images. GD Library is either not installed or not loaded in your server! Please install GD Library in your server or contact your hosting provider and ask them to install GD Library.");
}
...
function getAnnouncements() {
    ...
    if ($query->num_rows > 0)
    {
        while ($fetch = $query->fetch_array(MYSQLI_ASSOC))
        {
            ...
            $listAnnouncements .= \SocialKit\UI::view('announcements/list-each');
        }

        $themeData['list_announcements'] = $listAnnouncements;
        return \SocialKit\UI::view('announcements/content');
    }
}
...
function getStoryPostBox($timelineId=0, $recipientId=0, $placeholder='') /* Publisher, Wall Owner */
{
    ...
    if ($continue == true) {
        return \SocialKit\UI::view('story/publisher-box/content');
    }
}

/* Get follow suggestions */
function getFollowSuggestionsData($searchQuery='', $limit=5)
{
    if (! isLogged())
    {
        return array();
    ...
    AND id<>" . $user['id'] . "
    AND (id IN (SELECT id FROM " . DB_USERS . " WHERE follow_privacy='everyone')
...
function getFollowSuggestions($searchQuery='', $limit=5)
{
    if (! isLogged())
    {
        return array();
    ...
    foreach (getFollowSuggestionsData($searchQuery, $limit) as $k => $v)
    {
        $timelineObj = new \SocialKit\User();
        $timelineObj->setId($v);
        $timeline = $timelineObj->getRows();
        ...
        elseif ($timeline['type'] == "page")
        {
            ...
            $themeData['list_suggestion_info'] = $lang[$category['name']];
        }
        ...
        $suggestionsHtml .= \SocialKit\UI::view('suggestions/follow-suggestions-each');
    }

    $themeData['list_suggestions'] = $suggestionsHtml;
    return \SocialKit\UI::view('suggestions/follow-suggestions-content');
}
...
function getTrendings($type='latest', $limit=5)
{
    ...
    if ($query->num_rows > 0)
    {
        ...
        return \SocialKit\UI::view('trendings/content');
    }
}
...
function getNotifications($timelineId=0, $unread=false, $all=false)
{
    ...
    $conn->query("DELETE FROM " . DB_NOTIFICATIONS . " WHERE time<" . (time() - (60 * 60 * 24 * 5)) . " AND seen>0");
    return $get;
}
...
function getHashtagSearch($tag='', $limit=4)
{
    ...
    {
        $query = $conn->query("SELECT * FROM " . DB_HASHTAGS . " WHERE tag LIKE '%$tag%' LIMIT $limit");
    }
...
function getMessageRecipients($timelineId=0, $searchQuery='', $new=false)
{
    if (! isLogged())
    {
        return array();
    ...
    if (! empty($searchQuery))
    {
        $query = $conn->query("SELECT id FROM " . DB_ACCOUNTS . " WHERE id IN (SELECT following_id FROM " . DB_FOLLOWERS . " WHERE follower_id=$timelineId AND following_id NOT IN (SELECT id FROM " . DB_GROUPS . ") AND active=1) AND active=1 AND name LIKE '%$searchQuery%'");
    ...
    {
        $queryText = "SELECT id FROM " . DB_ACCOUNTS . " WHERE (id IN (SELECT timeline_id FROM " . DB_MESSAGES . " WHERE recipient_id=$timelineId AND active=1";
        ...
        if (! $new)
        {
            $queryText .= " OR id IN (SELECT recipient_id FROM " . DB_MESSAGES . " WHERE timeline_id=$timelineId AND active=1 ORDER BY id DESC)";
        }
...
function registerNotification($data=array())
{
    ...
    $url = $escapeObj->stringEscape($data['url']);
...
function deleteGroupMember($groupId=0, $memberId=0)
{
    ...
    if ($memberId == $user['id'] or $groupObj->isGroupAdmin())
    {
        ...
        if ($query)
        {
            $conn->query("DELETE FROM " . DB_GROUP_ADMINS . " WHERE admin_id=" . $memberId . " AND group_id=" . $groupId);
            return true;
        }
    }
}
...
function registerMessage($data=array())
{
    ...
    if ($post_ability)
    {
        $query = $conn->query("INSERT INTO messages (active,media_id,text,time,timeline_id,recipient_id) VALUES (1,$media_id,'$text'," . time() . ",$timelineId,$recipientId)");

        if ($query)
        {
            $post_id = $conn->insert_id;
            $query2 = $conn->query("UPDATE messages SET post_id=$post_id WHERE id=$post_id");
