...
if ($changeAvatar)
{
    $themeData['change_avatar_html'] = \SocialKit\UI::view('timeline/user/change-avatar');
}

if ($timeline['verified'] == true)
{
	$themeData['verified_badge'] = \SocialKit\UI::view('timeline/user/verified-badge');
}
...
if (isset($_GET['tab2']) && $_GET['tab2'] == "requests" && $owner == true)
{
    if ($config['friends'] == true)
    {
        ...
        $themeData['no_follow_requests_label'] = $lang['no_follow_requests'];
    }
    else
    {
        $themeData['follow_requests_label'] = $lang['follow_requests_label'];
        $themeData['no_follow_requests_label'] = $lang['no_friends'];
    }
    ...
    foreach ($timelineObj->getFollowRequests() as $requestId)
    {
        ...
        if ($owner == true)
        {
            $themeData['list_request_buttons'] = \SocialKit\UI::view('timeline/user/follow-request-button');
        }
...
$themeData['sidebar_postfilters'] = \SocialKit\UI::view('timeline/user/sidebar-post-filters');
