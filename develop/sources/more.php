...
foreach (getManagedPages() as $page) {
	...
	$listManagedPages .= \SocialKit\UI::view('more/list-page-each');
	$i++;
}

if ($i < 1) {
	$listManagedPages = \SocialKit\UI::view('more/no-pages');
}
...
foreach (getManagedGroups() as $group) {
	...
	$listManagedGroups .= \SocialKit\UI::view('more/list-group-each');
	$i++;
}

if ($i < 1) {
	$listManagedGroups = \SocialKit\UI::view('more/no-groups');
}
$themeData['list_managed_groups'] = $listManagedGroups;
/* */

$themeData['page_content'] = \SocialKit\UI::view('more/content');
