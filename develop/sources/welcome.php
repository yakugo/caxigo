...
for ($i = 1; $i < 32; $i++) {
    ...
    $listBirthDates .= \SocialKit\UI::view('welcome/list-signup-birth-date-each');
}
...
foreach (getMonths() as $month_number => $month_data) {
    ...
    $listBirthMonths .= \SocialKit\UI::view('welcome/list-signup-birth-month-each');
}
...
for ($i = date('Y')-100; $i < date('Y')-12; $i++) {
    ...
    $listBirthYears .= \SocialKit\UI::view('welcome/list-signup-birth-year-each');
}
$themeData['list_birth_years'] = $listBirthYears;

/* Sign Up Inputs */
if ($config['reg_req_birthday'] == true) {
    $themeData['signup_birthday_input'] = \SocialKit\UI::view('welcome/signup-birthday-input');
}

if ($config['reg_req_currentcity'] == true) {
    $themeData['signup_location_input'] = \SocialKit\UI::view('welcome/signup-location-input');
}

if ($config['reg_req_hometown'] == true) {
    $themeData['signup_hometown_input'] = \SocialKit\UI::view('welcome/signup-hometown-input');
}

if ($config['reg_req_about'] == true) {
    $themeData['signup_about_input'] = \SocialKit\UI::view('welcome/signup-about-input');
}

if ($config['captcha'] == true) {
    $captcha = createCaptcha();
    $themeData['captcha_image_src'] = $config['site_url'] . '/' . $captcha['image'];
    $themeData['signup_captcha_input'] = \SocialKit\UI::view('welcome/signup-captcha-input');
}
...
$themeData['reset_password_tab'] = \SocialKit\UI::view('welcome/reset-password-tab');
$themeData['reset_password_invalid'] = \SocialKit\UI::view('welcome/reset-password-invalid');
$themeData['forgot_password_tab'] = \SocialKit\UI::view('welcome/forgot-password-tab');
$themeData['login_tab'] = \SocialKit\UI::view('welcome/login-tab');
...
if ($signupTab == true) {
    $tabContent .= \SocialKit\UI::view('welcome/signup-tab');
}
