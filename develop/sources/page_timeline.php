...
if (isset($_GET['tab2']) && $_GET['tab2'] == "settings" && $isPageAdmin)
{
    ...
    {
		$themeData['admin_settings_link'] = \SocialKit\UI::view('timeline/page/admin/admin-settings-link');
	}

    $themeData['messages_count'] = $timelineObj->numMessages();

    $themeData['sidebar'] = \SocialKit\UI::view('timeline/page/admin/sidebar');
    ...
    elseif (isset($_GET['tab3']) && $_GET['tab3'] == "messages" && $isPageAdmin)
    {
        if (! empty($_GET['recipient_id']))
        {
            ...
            if (isset($recipient['id']))
            {
                ...
                if ($count_messages > 0)
                {
                    foreach ($messages as $msg)
                    {
                        ...
                        $listMessages .= \SocialKit\UI::view('messages/list-message-each');
                    }
            ...
            if ($no_messages == true) {
                $listMessages .= \SocialKit\UI::view('timeline/page/admin/no-messages');
            }
            ...
            if (is_array($message_recipients) && count($message_recipients) > 0) {
                ...
                foreach ($message_recipients as $recipient)
                {
                    ...
                    $messageRecipients .= \SocialKit\UI::view('timeline/page/admin/messages-default-each');
                }
                
            } else {
                $messageRecipients = \SocialKit\UI::view('timeline/page/admin/no-message-recipients');
            }
...
else
{
    ...
    $themeData['sidebar_postfilters'] = \SocialKit\UI::view('timeline/page/sidebar-post-filters');
