...
class User
{
	...
	public function getRows() {
		...
		if (isset($_SESSION['tempche']['user'][$this->id]))
		{
			$tempche = $_SESSION['tempche']['user'][$this->id];

			if ($tempche['expire_time'] > time())
			{
				$this->data = $_SESSION['tempche']['user'][$this->id];
				return $this->data;
			}
		}

		$query1 = $this->getConnection()->query("SELECT * FROM accounts WHERE id=" . $this->id . " AND active=1");
	...
	public function putNotification($a='')
    {
        ...
        if ($a == "follow")
        {
	        ...
	        $followingObj = new \SocialKit\User();
	        $followingObj->setId($followingId);
	        $count = $followingObj->numFollowers();
	        $text = '';
	...
	public function getAlbumsTemplate()
	{
		...
		foreach ($this->getAlbums() as $album)
		{
			...
			$listAlbums .= \SocialKit\UI::view('timeline/user/list-album-each');
		}
